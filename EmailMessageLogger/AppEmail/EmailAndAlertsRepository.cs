    ...
    public class EmailAndAlertsRepository
    {
        ...
        public void SendAlertsForCovenantsApproachingDueDateToRM(List<LoanCovenantDetailViewModel> loanDetails, string title)
        {
            try
            {
                ...
                {
                    string recipient = mailItem.EMAIL.Trim();
                    ...
                    string messageSubject = title; //" (REMINDER) - LOAN COVENANTS APPROACHING DUE DATE";
        ...
        public void SendAlertsForCovenantsApproachingDueDateToMonitoringTeam(List<LoanCovenantDetailViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                //var mailList = loanDetails.ToList();
                ...
                string messageSubject = alertSetups.MESSAGE_TITLE; //" (REMINDER) - LOAN COVENANTS APPROACHING DUE DATE";

                string messageContent = $"Dear Team, <br /><br />" +
                                 "This is to bring your attention the following loan covenants " +
                                 "which are approaching their due date for revaluation. <br /><br />" +
                                 $"{dataTable}";
        ...
        public void SendAlertsForCovenantsApproachingDueDate(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
            ...
            var loanDetails = (from a in context.TBL_LOAN_COVENANT_DETAIL
                               ...
                               where (int)DbFunctions.DiffDays(a.NEXTCOVENANTDATE,currentDate) <= alertsetupForCovenantsApproachingDueDate.NOTIFICATION_PERIOD1
        ...
        {
            var alertsetupForCovenantsOverDue = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.LoanCoveantOverdue).FirstOrDefault();
            ...
            var loanDetails = (from a in context.TBL_LOAN_COVENANT_DETAIL
                        ...
                        join f in context.TBL_FREQUENCY_TYPE on a.FREQUENCYTYPEID equals f.FREQUENCYTYPEID
                        join g in context.TBL_LOAN_COVENANT_TYPE on a.COVENANTTYPEID equals g.COVENANTTYPEID
                        where DbFunctions.DiffDays(a.NEXTCOVENANTDATE.Value, currentDate) >= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD1
                        select new LoanCovenantDetailViewModel
                        {
                            companyId = a.COMPANYID,
                            covenantAmount = a.COVENANTAMOUNT,
            ...
            {
                var escalationLevelOne = loanDetails.Where(x => x.notificationDuration <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD1).ToList();
            ...
            {
                var escalationLevelTwo = loanDetails.Where(x => x.notificationDuration <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD2).ToList();
            ...
            {
                var escalationLevelThree = loanDetails.Where(x => x.notificationDuration <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD3).ToList();
        ...
        public void SendAlertsForCovenantsOverDueRM(List<LoanCovenantDetailViewModel> loanDetails, string title)
        {
            try
            {
                ...
                foreach (var mailItem in staffList)
                {
                    ...
                    var additionalRecipient = loanDetails.FirstOrDefault(x => x.relationshipOfficerId == mailItem.STAFFID).officerEmail;
        ...
        public void SendAlertsForCovenantsOverDueMonitoringTeam(List<LoanCovenantDetailViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                string recipient = alertSetups.RECIPIENTEMAILS2.Trim();
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE;  //+ " (REMINDER) - LOAN COVENANTS OVERDUE";

                    var messageContent = $"Dear Team, <br /><br />" +
                                         "This is to bring your attention the following loan covenants " +
                                         "which are overdue for revaluation. <br /><br />" +
                                         $"{dataTable}";
        ...
        public void SendAlertsForCollateralPropertyRevaluation(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
            var alertsetupForCovenantsOverDue = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.CollateralRevaluation).FirstOrDefault();
            ...
            var loanDetails = (from a in context.TBL_COLLATERAL_CUSTOMER
                               ...
                               where (DbFunctions.DiffDays(DbFunctions.AddDays(f.LASTVALUATIONDATE, a.VALUATIONCYCLE), currentDate) <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD1)
                               select new CollateralViewModel
                               {
                                   ...
                                   notificationDuration = (int)DbFunctions.DiffDays(DbFunctions.AddDays(f.LASTVALUATIONDATE, a.VALUATIONCYCLE), currentDate)
        ...
        public void SendAlertsForCollateralPropertyRevaluationRM(List<CollateralViewModel> loanDetails, string title)
        {
            try
            {
                ...
                foreach (var mailItem in staffList)
                {
                    ...
                    string messageSubject = title; //+ " (REMINDER) - COLLATERAL DUE FOR RE-EVALUATION";
        ...
        public void SendAlertsForCollateralPropertyRevaluationMonitoringTeam(List<CollateralViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                string recipient = alertSetups.RECIPIENTEMAILS2.Trim();
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE; //+ " (REMINDER) - COLLATERAL DUE FOR RE-EVALUATION";
        ...
        public void SendAlertsForLoanNplMonitoring(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
            var alertsetupForCovenantsOverDue = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.CollateralRevaluation).FirstOrDefault();
            ...
            {
                var escalationLevelOne = loanDetails.Where(x => x.notificationDuration <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD1).ToList();
                SendAlertsForLoanNplMonitoringMonitoringTeam(escalationLevelOne, alertsetupForCovenantsOverDue);
            }
            if (alertsetupForCovenantsOverDue.RECIPIENTEMAILS2.Trim() != string.Empty)
            {
                var escalationLevelTwo = loanDetails.Where(x => x.notificationDuration <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD2).ToList();
                SendAlertsForLoanNplMonitoringMonitoringTeam(escalationLevelTwo, alertsetupForCovenantsOverDue);
            }
            if (alertsetupForCovenantsOverDue.RECIPIENTEMAILS3.Trim() != string.Empty)
            {
                var escalationLevelThree = loanDetails.Where(x => x.notificationDuration <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD3).ToList();
                SendAlertsForLoanNplMonitoringMonitoringTeam(escalationLevelThree, alertsetupForCovenantsOverDue);
            }
        ...
        public void SendAlertsForLoanNplMonitoringMonitoringTeam(List<LoanViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                dataTable =
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE; //+ " (REMINDER) - NON-PERFORMING LOANS";
        ...
        public void SendAlertsOnSelfLiquidatingLoanExpiry(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
            var alertsetupForCovenantsOverDue = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.SelfLiquidatingLoanExpiry).FirstOrDefault();
            ...
            var data = (from a in context.TBL_LOAN
                        ...
                        DbFunctions.DiffDays(a.MATURITYDATE, applDate) <= alertsetupForCovenantsOverDue.NOTIFICATION_PERIOD1
                        select new LoanViewModel
                        {
        ...
        public void SendAlertsOnSelfLiquidatingLoanExpiryRM(List<LoanViewModel> loanDetails, string title)
        {
            try
            {
                ...
                foreach (var mailItem in staffList)
                {
                    ...
                    var otherRecipient = loanDetails.FirstOrDefault(x => x.relationshipOfficerId == mailItem.STAFFID).relationshipOfficerEmail;
        ...
        public void SendAlertsOnSelfLiquidatingLoanExpiryMonitoringTeam(List<LoanViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                string messageSubject = alertSetups.MESSAGE_TITLE; //+ " (REMINDER) - EXPIRED SELF-LIQUIDATING LOANS";
        ...
        public void SendAlertsOnOverDraftLoansAlmostDue(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
            var alertsetupForCovenantsOverDue = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.CollateralRevaluation).FirstOrDefault();
        ...
        public void SendAlertsOnOverDraftLoansAlmostDueMonitoringTeam(List<LoanViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                string recipient = alertSetups.RECIPIENTEMAILS2.Trim();
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE; //+ " (REMINDER) - EXPIRED OVERDRAFT LOANS";
        ...
        public void SendAlertsForExpiredInsurance(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
            var alertsetupForCovenantsOverDue = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.CollateralRevaluation).FirstOrDefault();
        ...
        public void SendAlertsForExpiredInsuranceRM(List<CollateralViewModel> loanDetails, string title)
        {
            try
            {
                var staffList = context.TBL_STAFF.ToList();
        ...
        public void SendAlertsForExpiredInsuranceMonitoringTeam(List<CollateralViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                string dataTable;
                ...
                string recipient = alertSetups.RECIPIENTEMAILS2.Trim();
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE;  //+ " (REMINDER) - EXPIRED INSURANCE";
        ...
        //#######################        LOAN CASA WITH PND         ############################ 
        public void SendAlertsOnLoanCASAwithPND(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
           // var CASAwithPND = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum.CASAwithPND).FirstOrDefault();
        ...
        public void SendAlertsOnLoanCASAwithPNDrm(List<LoanViewModel> loanDetails, string title)
        {
            try
            {
                ...
                foreach (var mailItem in staffList)
                {
                    ...
                    string messageSubject = title;// + " (REMINDER) - LOAN CASA WITH PND AND LEIN";
        ...
        public void SendAlertsOnLoanCASAwithPNDmonitoringTeam(List<LoanViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                ...
                string recipient = alertSetups.RECIPIENTEMAILS2.Trim();
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE;// + " (REMINDER) - LOAN CASA WITH PND AND LEIN";
        ...
        public void SendAlertsOnLoanForInActiveBondAndGuarantee(string title, string messageBody, List<TBL_MONITORING_ALERT_SETUP> alertSetups)
        {
          //  var CASAwithPND = alertSetups.Where(x => x.MONITORING_ITEMID == (int)AlertMessageEnum).FirstOrDefault();
        ...
        public void SendAlertsOnLoanForInActiveBondAndGuaranteeRM(List<LoanViewModel> loanDetails, string title)
        {
            try
            {
                var staffList = staffRepo.GetAllStaff().ToList();
        ...
        public void SendAlertsOnLoanForInActiveBondAndGuaranteeMonitoringTeam(List<LoanViewModel> loanDetails, TBL_MONITORING_ALERT_SETUP alertSetups)
        {
            try
            {
                string dataTable;
                ...
                string recipient = alertSetups.RECIPIENTEMAILS2.Trim();
                    ...
                    string messageSubject = alertSetups.MESSAGE_TITLE;  ///+ " (REMINDER) - INACTIVE BOND AND GUARANTEE";
