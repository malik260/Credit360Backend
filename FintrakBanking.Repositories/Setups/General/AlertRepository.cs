...
namespace FintrakBanking.Repositories.Setups.General
{
    public class AlertRepository : IAlertRepository
    {
        ...
        public IEnumerable<AlertTitleViewModel> GetAllAlertsForDropdown()
        {
            ...
            var alertsMethods = (from c in context.TBL_ALERT_BINDING_METHODS
                                 ...
                                 select new AlertTitleViewModel
                                 {
                                     ...
                                     lastSentDate = a.LASTSENTDATE,
                                     actionStatus = a.ACTIONSTATUS,
                                     isActive = a.ISACTIVE == false ? true : true,
                                     alertScheduleId = b.ALERTSCHEDULEID,
                                     frequencyId = b.FREQUENCYID,
        ...
        public bool AddAlertTitle(AlertTitleViewModel model)
        {
            ...
            {
                var rec2 = context.TBL_ALERT_TITLE.Where(x => x.ALERTTITLEID == model.bindingMethodId).FirstOrDefault();
        ...
        public bool UpdateAlertTitleStatus(AlertTitleViewModel model)
        {
            ...
            var entity = context.TBL_ALERT_TITLE.Find(model.alertTitleId);
            if (entity != null)
            {
       ...
       /* public static bool CompareHash(string attemptedPassword, byte[] hash, int salt)
        ...
        private bool CompareCustomerNotificationDate()
        {
            ...
            var DBdate = context.TBL_MESSAGE_LOG.Where(m => DbFunctions.TruncateTime(m.SENDONDATETIME) == DbFunctions.TruncateTime(currentDate)
                         && (m.OPERATIONMETHOD.Trim() == "GetLoanRepaymentReminder"
                         || m.OPERATIONMETHOD.Trim() == "GetRepaymentPastDueForCustomers"
                         )).FirstOrDefault();
        ...
        public void GetImminentMaturitiesForCustomers()
        {
            ...
            var customerList = context.TBL_ALERT_CUSTOMER_CODE.Where(d => d.CUSTOMERCODE != null).Select(d=>d.CUSTOMERCODE).ToList();
        ...
        public void GetRepaymentPastDueForCustomers()
        {
            // GetLoanRepaymentReminder method
            var customerList = context.TBL_ALERT_CUSTOMER_CODE.Where(d => d.CUSTOMERCODE != null).Select(d => d.CUSTOMERCODE).ToList();
        ...
        public void GetPastDueObligationsReminder()
        {
            ...
            if (pastDueObligationsReminder != null && pastDueObligationsReminder.Count() > 0)
            {
                ...
                foreach (var staff in pastDueObligationsReminder)
                {
                    ...
                    var loanInformation = context.TBL_GLOBAL_EXPOSURE.Where(d => d.UNPODAYSOVERDUE > 0 && d.ACCOUNTOFFICERCODE == staff.misCode && d.TOTALUNSETTLEDAMOUNT > 0).ToList();
                    if (loanInformation != null && loanInformation.Count() > 0)
                    {
                        var n = 0;
                        var result = $@"
        ...
        public Tuple<List<MultipleCustomerCodeOutputViewModel>, bool> preBulkCustomerCode(byte[] file, UserInfo user, bool isFinal)
        {
            List<TBL_ALERT_CUSTOMER_CODE> insurance = new List<TBL_ALERT_CUSTOMER_CODE>();
            List<MultipleCustomerCodeOutputViewModel> insuranceInputs = GetBulkCustomerCodeInputs(file);

            return new Tuple<List<MultipleCustomerCodeOutputViewModel>, bool>(insuranceInputs, true);
        }

        private List<MultipleCustomerCodeOutputViewModel> GetBulkCustomerCodeInputs(byte[] file)
        {
            List<MultipleCustomerCodeOutputViewModel> bulkEntries = new List<MultipleCustomerCodeOutputViewModel>();
            ...
            {
                MultipleCustomerCodeOutputViewModel currentLine = new MultipleCustomerCodeOutputViewModel();
                int ctr = 0;
            ...
            foreach (var currentLine in bulkEntries)
            {
                ...
                {
                    //========================================== other valiadation =============================
                    var customer = context.TBL_CUSTOMER.Where(x => x.CUSTOMERCODE == currentLine.customerCode).FirstOrDefault();
                    if (customer == null)
                    {
                        currentLine.passed = false;
                        currentLine.errorMessages.Add("Customer with customercode " + currentLine.customerCode.ToString() + " does not exist on Credit360");
                    }

                    var customerCodeExist = context.TBL_ALERT_CUSTOMER_CODE.Where(x => x.CUSTOMERCODE == currentLine.customerCode).FirstOrDefault();
                    if (customerCodeExist != null)
                    {
                        currentLine.passed = false;
                        currentLine.errorMessages.Add("Customer with customercode " + currentLine.customerCode.ToString() + " already exist on Credit360");
                    }
        ...
        public bool saveBulkInsurancePolicyEntries(List<MultipleCustomerCodeOutputViewModel> models, UserInfo user)
        {
            List<TBL_ALERT_CUSTOMER_CODE> bulkCustomerCodeTable = new List<TBL_ALERT_CUSTOMER_CODE>();
            
            using (TransactionScope transactionScope = new TransactionScope())
            {
                foreach (var dataRequest in models)
                {
                    if (dataRequest.customerCode.ToLower() != "")
                    {
                        var confirmIfRecordAlreadyExist = context.TBL_ALERT_CUSTOMER_CODE.Where(t => t.CUSTOMERCODE == dataRequest.customerCode).FirstOrDefault();
                        if (confirmIfRecordAlreadyExist == null)
                        {
                            dataRequest.dateTimeCreated = DateTime.Now;
                            var customerCodeData = addBulkCustomerCode(dataRequest);
                            bulkCustomerCodeTable.Add(customerCodeData);

                        }
                    }
                }
                context.TBL_ALERT_CUSTOMER_CODE.AddRange(bulkCustomerCodeTable);
        ...
        private TBL_ALERT_CUSTOMER_CODE addBulkCustomerCode(MultipleCustomerCodeOutputViewModel request)
        {
            var insuranceTracking = context.TBL_ALERT_CUSTOMER_CODE.Add(new TBL_ALERT_CUSTOMER_CODE
            {
