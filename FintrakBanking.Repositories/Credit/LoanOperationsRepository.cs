    ...
    public class LoanOperationsRepository : ILoanOperationsRepository
    {
        ...
        public IEnumerable<DailyInterestAccrualViewModel> ProcessDailyFeeAccrual(DateTime applicationDate)
        {
            ...
            try
            {
                ...
                var data1 = (from a in context.TBL_LOAN_FEE
                             join b in context.TBL_LOAN on a.LOANID equals b.TERMLOANID
                             join d in context.TBL_DAY_COUNT_CONVENTION on b.SCHEDULEDAYCOUNTCONVENTIONID equals d.DAYCOUNTCONVENTIONID
                ...
                var data2 = (from a in context.TBL_LOAN_FEE
        ...
        public IEnumerable<DailyInterestAccrualViewModel> ProcessDailyInterestOnPastDueInterestAccrual(DateTime applicationDate)
        ...
        public IEnumerable<DailyInterestAccrualViewModel> ProcessDailyInterestOnPastDuePrincipalAccrual(DateTime applicationDate)
        ...
        public bool UpdateLoanPrepaymentSchedule(TwoFactorAutheticationViewModel twoFactorAuth, int loanId, LoanPaymentRestructureScheduleInputViewModel loanInput, DateTime applicationDate, int staffId)
        {
...

            {
                ...
                if (loanInput.payAmount >= (double)(totalamount))
                {
                    ...
                    if (pastDue != 0)
                    {
                        inputTransactions.Add(financeTransaction.PostBuildLoanPrepaymentPosting(loanInput, twoFactorAuth, pastDue, product.INTERESTRECEIVABLEPAYABLEGL.Value, "Past Due", (int)OperationsEnum.InterestPastDueLoanRepayment));
                    }
                ...
                else
                {
                    ...
                    if (LoanExist(loanId) > 0)
                    {
                        ...
                        loanInput.principalAmount = ((double)totalamount - (double)partPayment);
        ...
        public bool LoanBackDateFunction(int loanId, DateTime effectiveDate, DateTime currentDate, decimal pastDueRate, LoanPaymentRestructureScheduleInputViewModel loanInput)
        {
            ...
            decimal previousAccruedInterest = (decimal?)previousDailyInterest.FirstOrDefault() ?? 0;
            decimal.Round(previousAccruedInterest, 2, MidpointRounding.AwayFromZero);

            var currentDailyInterest = from d in context.TBL_LOAN_SCHEDULE_DAILY_TEMP
                        where d.LOANID == loanId
            ...
            decimal currentAccruedInterest = (decimal?)currentDailyInterest.FirstOrDefault() ?? 0;
            ...
            var currentPeriodic = from d in context.TBL_LOAN_SCHEDULE_PERIODIC_TMP
                        where d.LOANID == loanId
            ...
            var currentPeriodicForPrincipal = from d in context.TBL_LOAN_SCHEDULE_PERIODIC_TMP
                        where d.LOANID == loanId
            ...
            if (Count < 1 && effectiveDate == currentDate)
            {
            }
            else if (Count < 1 && effectiveDate <= currentDate)
            {

                inputTransactions.Add(financeTransaction.PostBuildLoanPositiveReversalPosting(loanInput, accruedInterestDiff, product.INTERESTRECEIVABLEPAYABLEGL.Value, "Accrued Interest Reversal", loanInput.operationId)); //change later));

                //result = financeTransaction.PostTransaction(inputTransactions);
            }
            else
            {
                inputTransactions.Add(financeTransaction.PostBuildLoanPositiveReversalPosting(loanInput, accruedInterestDiff, product.INTERESTRECEIVABLEPAYABLEGL.Value, "Accrued Interest Reversal", loanInput.operationId));
        ...
        public bool LoanTermination(int loanId, LoanPaymentRestructureScheduleInputViewModel loanInput, TwoFactorAutheticationViewModel twoFactorAuth, DateTime applicationDate, int staffId)
        {
            ...
            try
            {
                ...
                if (pastDue > 0)
                {
                    ...
                    inputTransactions.Add(financeTransaction.BuildTerminateAndRebookPosting(loanId, loanInput, pastDue, product.INTERESTRECEIVABLEPAYABLEGL.Value, "Past Due Interests"));
        ...
        private bool ProcessContingentLiabilityRenewal(TwoFactorAutheticationViewModel twoFactorAuth, LoanPaymentRestructureScheduleInputViewModel model, string approvalComment)
        {
            ...
            if (DoesOperationExist(model.loanId, model.operationId))
            {
                throw new ConditionNotMetException("The requested operation already exist and going through approval");
            }
        ...
        public bool addNonTermLoanLoanRateChange(LoanReviewViewModel userModel)
        {
            ...
            if (userModel.loanId != 0)
            {
                ...
                if (userModel.valueDate < systemDate)
                {
                    ...
                    {
                        int daysDiff = loanGenerate.getDaysInLoanPeriod(userModel.valueDate, systemDate);

                        double previousAmount = (((loanRecord.INTERESTRATE / 100) * (double)loanRecord.PRINCIPALAMOUNT * 1 / loanGenerate.getDaysInLoanPeriod(loanRecord.EFFECTIVEDATE, loanRecord.MATURITYDATE)) * daysDiff);
                        double currentAmount = (((userModel.newRate / 100) * (double)loanRecord.PRINCIPALAMOUNT * 1 / loanGenerate.getDaysInLoanPeriod(loanRecord.EFFECTIVEDATE, loanRecord.MATURITYDATE)) * daysDiff);
        ...
        public loanPrepaymentViewModel addCommercialLoanPrepayment(string refNo, loanPrepaymentViewModel model)
        {
            ...
            if (model.effectiveDate < systemDate)
            {
                ...
                inputTransactions.Add(financeTransaction.PostBuildLoanPositiveReversalPosting(payStructureModel, accruedInterestDiff, loanRecord.TBL_PRODUCT.INTERESTRECEIVABLEPAYABLEGL.Value, "Accrued Interest Reversal", (short)OperationsEnum.Prepayment)); //change later));
                //**************************************************************************
