...
namespace FintrakBanking.Repositories.Credit
{
    public class FacilityDetailSummary : IFacilityDetailSummary
    {
        ...
        private IEnumerable<CamProcessedLoanViewModel> AvailedLoanApplicationsDetails(int companyId, int staffId, int branchId,string refNo)
        {
            try
            {
                var data = (from d in context.TBL_LOAN_APPLICATION_DETAIL
                            ...
                            && m.BRANCHID == branchId
                            && m.APPLICATIONREFERENCENUMBER.Contains(refNo)
                ...
                {
                    var loans = context.TBL_LOAN.Where(tl => tl.LOANAPPLICATIONDETAILID == item.loanApplicationDetailId).ToList();
                    var overdrafts = context.TBL_LOAN_REVOLVING.Where(tl => tl.LOANAPPLICATIONDETAILID == item.loanApplicationDetailId).ToList();
                    var contingents = context.TBL_LOAN_CONTINGENT.Where(tl => tl.LOANAPPLICATIONDETAILID == item.loanApplicationDetailId).ToList();
                    switch (item.productTypeId)
                    {
                        ...
                        case (short)LoanProductTypeEnum.CommercialLoan:
                           ...
                           // item.loanId = loans.FirstOrDefault().TERMLOANID;
                            break;
                        ...
                        case (short)LoanProductTypeEnum.RevolvingLoan:
                          ...
                          //  item.loanId = overdrafts.FirstOrDefault().REVOLVINGLOANID;
                            break;
                        case (short)LoanProductTypeEnum.ContingentLiability:
                           ...
                           // item.loanId = contingents.FirstOrDefault().CONTINGENTLOANID;
                            break;
                    }
                }
                return data.ToList();
        ...
        public IEnumerable<CamProcessedLoanViewModel> GetLoanFacilityUtilization(int companyId, int staffId, int branchId, string searchValue = null)
        {
            try
            {
                ...
                var data = (from d in context.TBL_LOAN_APPLICATION_DETAIL
                            ...
                            && m.APPLICATIONSTATUSID != (short)LoanApplicationStatusEnum.CancellationCompleted
                            && m.BRANCHID == branchId
                            && m.APPLICATIONREFERENCENUMBER.Contains(searchValue)
        ...
        public LoanViewModel GetLoanFacilityDetail(int loanApplicationDetilId)
        {
            var loan = (from x in context.TBL_LOAN
                       where x.LOANAPPLICATIONDETAILID == loanApplicationDetilId
                       select new LoanViewModel
                       {
                           loanId = x.TERMLOANID,
                           loanSystemTypeId = x.LOANSYSTEMTYPEID
            ...
            if (loan==null)
            {
                loan = (from x in context.TBL_LOAN_CONTINGENT
                        where x.LOANAPPLICATIONDETAILID == loanApplicationDetilId
                        select new LoanViewModel
                        {
            ...
            if (loan == null)
            {
                loan = (from x in context.TBL_LOAN_REVOLVING
                        where x.LOANAPPLICATIONDETAILID == loanApplicationDetilId
                        select new LoanViewModel
                        {
