...
namespace FintrakBanking.Repositories.Credit
{
    public partial class LoanApplicationRepository : ILoanApplicationRepository
    {
        ...
        public LoanApplicationRepository(IAuditTrailRepository _auditTrail, ICasaRepository _casa,
        ...
        public IQueryable<LoanApplicationViewModel> GetPendingLoanApplications(int companyId, int branchId, int staffId)
        {
            ...
            {
                return context.tbl_Loan_Application.Where(
                        x => x.CompanyId == companyId && x.Deleted == false //&& x.BranchId == branchId
                    )
                    .Select(a => new LoanApplicationViewModel
                    {
                        ...
                        customerName =
                            a.CustomerId.HasValue
                                ? a.tbl_Customer.FirstName + " " + a.tbl_Customer.MiddleName + " " +
                                  a.tbl_Customer.LastName
                                : "",
                        loanInformation = a.LoanInformation,
                        companyId = a.CompanyId,
                        branchId = a.BranchId,
            ...
            if (scope == (int)ProcessViewScopeEnum.Group) // 2
            {
                ...
                pendingApplications =
                    pendingApplications.Where(x => groupApprovalLevelIds.Contains((int)x.b.ToApprovalLevelId));
            }

            if (scope == (int)ProcessViewScopeEnum.Level) // 1
            {
                pendingApplications =
                    pendingApplications.Where(x => staffApprovalLevelIds.Contains((int)x.b.ToApprovalLevelId));
            }
        ...
        public bool CheckExistingCertificateOfOwnership(string certificateOfOwnership, int companyId)
        {
            ...
            var collate = collateral.GetCustomerCollateral(companyId)
                .Where(c => c.collateralCode == certificateOfOwnership);
            if (collate.Any())
            {
                return true;
            }
            ...
            var data = (from a in context.tbl_Loan_Application
                        ...
                        select new CamProcessedLoanViewModel
                        {
                            ...
                            exchangeRate = a.ExchangeRate,
                                               ...
                                               || (context.tbl_Customer_Group_Mapping.Count(x =>
                                                       x.CustomerGroupId == r.customerGroupId)
                                                   >= context.tbl_Loan.Count(g => g.CustomerGroupId == r.customerGroupId) &&
