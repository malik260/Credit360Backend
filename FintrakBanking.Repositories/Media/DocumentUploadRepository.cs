...
namespace FintrakBanking.Repositories.Media
{
    public class DocumentUploadRepository : IDocumentUploadRepository
    {
        ...
        public IEnumerable<DocumentUploadViewModel> GetDocumentUploads(int staffId, int operationId, int targetId, bool isOperationSpecific=false, bool isLms = false)
        {
            //if (isLms)
            //{
            //    var lmsFirstQuery = docContext.TBL_DOCUMENT_USAGE.Where(x => x.DELETED == false && x.OPERATIONID == operationId && x.TARGETID == targetId)
